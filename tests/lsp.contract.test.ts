...
describe('lsp canonical contracts', () => {
  it('provides canonical keyword and statement completions', () => {
    ...
    expect(completions.some((c) => c.label === 'route(...);' && c.kind === 'advanced')).toBe(false);
    expect(completions.some((c) => c.label === 'ROUT' && c.kind === 'register')).toBe(true);

    const guardCompletions = getCompletions('gu');
    expect(guardCompletions.some((c) => c.label === 'std::guard(...);' && c.kind === 'advanced')).toBe(true);

    const collectCompletions = getCompletions('co');
    expect(collectCompletions.some((c) => c.label === 'std::collect(...);' && c.kind === 'advanced')).toBe(true);

    const stashCompletions = getCompletions('st');
    expect(stashCompletions.some((c) => c.label === 'std::stash(...);' && c.kind === 'advanced')).toBe(true);

    const accumulateCompletions = getCompletions('ac');
    expect(accumulateCompletions.some((c) => c.label === 'std::accumulate(...);' && c.kind === 'advanced')).toBe(true);

    const mulaccCompletions = getCompletions('mulacc');
    expect(mulaccCompletions.some((c) => c.label === 'std::mulacc_chain(...);' && c.kind === 'advanced')).toBe(true);

    const carryChainCompletions = getCompletions('carry');
    expect(carryChainCompletions.some((c) => c.label === 'std::carry_chain(...);' && c.kind === 'advanced')).toBe(true);

    const conditionalSubCompletions = getCompletions('cond');
    expect(conditionalSubCompletions.some((c) => c.label === 'std::conditional_sub(...);' && c.kind === 'advanced')).toBe(true);

    const normalizeCompletions = getCompletions('no');
    expect(normalizeCompletions.some((c) => c.label === 'std::normalize(...);' && c.kind === 'advanced')).toBe(true);

    const extractCompletions = getCompletions('ex');
    expect(extractCompletions.some((c) => c.label === 'std::extract_bytes(...);' && c.kind === 'advanced')).toBe(true);

    const latencyHideCompletions = getCompletions('lat');
    expect(latencyHideCompletions.some((c) => c.label === 'std::latency_hide(...);' && c.kind === 'advanced')).toBe(true);
  ...
  it('reports parse errors for unsupported syntax', () => {
    const source = `
