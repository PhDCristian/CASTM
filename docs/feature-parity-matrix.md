...
| `latency_hide` scheduling (`FEAT-1`, `OPT-A` baseline) | done | `tests/issues/feat-01-latency-hide.test.ts`, `tests/compiler-api.latency-hide.test.ts`, `tests/issues/opt-a-cycle-packing.test.ts` | Conservative post-expansion cycle packing merges adjacent independent cycles only when hazard guards pass (PE overlap, direct route-hop dependency, control barrier, dual-memory adjacency). |
...
| `accumulate` lowering (`FEAT-13`) | done | `tests/issues/feat-13-accumulate.test.ts` | Canonical accumulation pattern `accumulate(pattern=row|col|anti_diagonal, products=..., accum=..., out=...)` lowers deterministically across NxM grids with explicit stage ordering. |
