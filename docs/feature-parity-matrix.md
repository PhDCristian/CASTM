...
| `latency_hide` scheduling (`FEAT-1`) | done | `tests/issues/feat-01-latency-hide.test.ts`, `tests/compiler-api.latency-hide.test.ts` | Conservative post-expansion compaction merges adjacent cycles only when hazard guards pass (PE overlap, route boundary, control barrier, dual-memory adjacency). |
