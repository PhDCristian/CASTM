# Migrating from Legacy OpenEdgeDSL

This guide describes how to move stable integrations from legacy OpenEdgeDSL to the package-based architecture.
...
### After (package-based architecture)
...
The compiler returns structured artifacts and diagnostics, including:
- `csv`
- `ast` / `hir` / `mir` / `lir`
- `memoryRegions`
- `ioConfig`
...
Stable legacy language/pragmas/directives are available in the current compiler (see `docs/feature-parity-matrix.md`).
...
4. Switch default backend after CI parity is green.
5. Remove legacy fallback path; if UI keeps `legacy` setting compatibility, map it explicitly to the current compiler backend.
