...
import { expandSpatialAtBlockStatements } from '../bundle-spatial.js';
...
export function tryExpandSingleBundleStatementStep(input: ExpandLoopEntryInput): ExpandLoopEntryResult {
  if (input.clean === '}') {
    input.diagnostics.push(makeDiagnostic(
      ErrorCodes.Parse.InvalidSyntax,
      'error',
  ...
  for (const candidate of candidates) {
    const parsedStatements = parseBundleStatement(
      candidate,
      input.entry.lineNo,
      input.raw,
      input.constants,
