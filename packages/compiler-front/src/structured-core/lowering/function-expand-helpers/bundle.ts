...
export function bundleHasControlFlow(bundle: BundleAst): boolean {
  if (bundle.label) return true;
...
export function cloneBundle(bundle: BundleAst, index: number): BundleAst {
  return {
    index,
    label: bundle.label,
    span: { ...bundle.span },
    statements: bundle.statements.map((statement) => {
...
export function makeControlBundle(
  index: number,
  lineNo: number,
  row: number,
  col: number,
...
/**
 * Creates a single bundle containing both the SADD (set return address)
 * and JUMP (call entry) instructions on two different PEs.
 * This merges the call setup into one bundle, mimicking v15-style
 * multi-PE JUMP merging.
 */
export function makeCallBundle(
  index: number,
  lineNo: number,
  jumpRow: number,
  jumpCol: number,
