...
function collectStatementPlacements(statement: BundleStatementAst, grid: GridSpec): Placement[] | null {
...
function summarizeBundle(bundle: BundleAst, grid: GridSpec): BundleSummary | null {
  const placements: Placement[] = [];
...
function hasRouteDependency(
  producer: BundleSummary,
  consumer: BundleSummary,
  grid: GridSpec
): boolean {
...
function canMergeBundles(current: BundleSummary, next: BundleSummary, grid: GridSpec): boolean {
...
function mergedBundleFromSummaries(
  currentBundle: BundleAst,
...
export function applyLatencyHide(
  ...
  if (!Number.isInteger(window) || window <= 0 || bundles.length <= 1) {
    return bundles.map((bundle, index) => ({ ...bundle, index }));
  }

  const compacted = bundles.map((bundle) => ({
    ...bundle,
    statements: [...bundle.statements]
  }));

  let bundleIndex = 0;
  while (bundleIndex < compacted.length - 1) {
    let mergedCount = 0;
    while (mergedCount < window && bundleIndex < compacted.length - 1) {
      const nextBundle = compacted[bundleIndex + 1];
      // Never absorb a labeled bundle as the "next" in a merge — its label
      // would be lost because mergedBundleFromSummaries only preserves the
      // label of the *current* (left) bundle.  Labeled bundles must stay as
      // the head of their own merge group so their label is retained.
      if (nextBundle.label) break;
      const currentSummary = summarizeBundle(compacted[bundleIndex], grid);
      const nextSummary = summarizeBundle(nextBundle, grid);
      if (!currentSummary || !nextSummary) break;
      if (!canMergeBundles(currentSummary, nextSummary, grid)) break;

      compacted[bundleIndex] = mergedBundleFromSummaries(
        compacted[bundleIndex],
        currentSummary,
        nextSummary
      );
      compacted.splice(bundleIndex + 1, 1);
      mergedCount++;
    }
    bundleIndex++;
  }

  return compacted.map((bundle, index) => ({
    ...bundle,
    index
  }));
}
